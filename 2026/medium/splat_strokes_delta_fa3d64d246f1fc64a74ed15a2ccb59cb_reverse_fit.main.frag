...
#else
    // Plain mesh fallback (--viewer's reference figure/floor): v_uv/v_uvStep
    // are only meaningful for the real per-mark splat's uv-encoded color
    // bytes, so skip the index-carve/aligned-photo lookup above and shade it
    // as a basic lit surface instead.
