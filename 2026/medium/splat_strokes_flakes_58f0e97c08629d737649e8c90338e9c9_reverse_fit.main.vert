...
void main() {
    ...
    // The splat's 3D covariance (world). Both the rendered quad's orientation
    // and the source-image lookup are derived from the mark's own axes recovered
    // from this, so they share one basis and can't disagree.
    mat3 Vrk = mat3(
        p2.x, p2.y, p2.z,
        p2.y, p2.w, p3.x,
        p2.z, p3.x, p3.y
    ...
    float rnd = fract(fIndex * 0.12345);
    float pct = u_time * 0.5 + (cam.z * 3. + rnd * 0.125);
    v_pct = 0.5 + 0.5 * sin(pct);
    // v_pct = smoothstep(0.0, 1.0, v_pct);
    ...
    vec3 axis2 = s2 * (-sin(phi + v_pct * PI) * t1 + cos(phi + v_pct * PI) * t2);        // world minor half-axis
    ...
    float scale = mix(1.2, 1.75, v_pct);
