...
#define MAX_EXTRA_RADIUS 0.005
#define SPLAT_SCALE 1.2
#define UV_FILL 0.5

// Decode a unit vector packed with signed octahedral encoding (mirrors vera's
// octEncode in gsplat.cpp -- p3.zw carries the splat's local/world thin axis).
...
void main() {
...
#elif defined(MODEL_PRIMITIVE_TRIANGLES)
    ...
    #ifdef MODEL_VERTEX_NORMAL
    worldNormal = normalize(u_normalMatrix * a_normal);
    v_normal = worldNormal;
    #endif

    // Vertex-shader texture fetch -- same pattern shaders/paths/main.vert
    // already uses (sampling u_xy
    float bright = luma(texture2D(u_alignedTex, v_texcoord).rgb);
    float widthT = 1. - bright; // darker photo region -> thicker line, engraving-style
    
    // v_widthT = widthT;
    v_position.xyz += worldNormal * (MAX_EXTRA_RADIUS * widthT);
    v_color = texture2D(u_alignedTex, v_texcoord);
    ...
    #if defined(SCENE_BUFFER_NORMAL) && !defined(MODEL_PRIMITIVE_TRIANGLES)
    gl_Position = vec4(0.0);
    gl_PointSize = 0.0;
    #endif
#endif
